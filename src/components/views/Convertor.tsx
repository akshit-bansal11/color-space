...
function GradientClassConverter() {
  const [mode, setMode] = useState<"tw-to-css" | "css-to-tw">("tw-to-css");
  ...
  useEffect(() => {
    ...
    if (mode === "css-to-tw" && inputs.cssType === "angle") {
      ...
      else if (!isCssAngle(inputs.cssAngle))
        e.cssAngle = "Invalid (e.g., 45deg).";
    }
    setErrors(e);
  }, [inputs, mode]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2">
        ...
        <motion.button
          onClick={() =>
            setMode(mode === "tw-to-css" ? "css-to-tw" : "tw-to-css")
          }
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        {inputFields.map((field) =>
          renderInputField({ ...field, onChange: handle }),
        )}
      </div>
      ...
      <AnimatePresence>
        {result && (
          <ResultBox
            label="Result"
            value={result}
            className={previewGradient ? "mt-4" : ""}
          />
        )}
      </AnimatePresence>
    </motion.div>
