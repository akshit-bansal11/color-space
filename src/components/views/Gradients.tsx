...
interface GradientCardProps extends Gradient {}

function GradientCard({ name, feel, tailwind, css }: GradientCardProps) {
  const [copied, copy] = useCopyToClipboard();
  const [copiedKey, setCopiedKey] = useState<string | null>(null); // tracks which text was copied
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="lg:text-xl font-extrabold font-chillax text-white">
          {name}
        </h3>
...
export default function GradientsBrowser() {
  ...
  const feels = useMemo(
    () => ["All", ...Array.from(new Set(gradients.map((g) => g.feel)))],
    [],
  );
          ...
          <div className="flex gap-2 flex-wrap">
            {feels.map((feel) => (
              <button
                ...
                className={`px-3 py-1 text-sm rounded-full transition-colors ${feelFilter === feel ? "bg-blue-600 text-white" : "bg-neutral-700 text-neutral-300 hover:bg-neutral-600"}`}
              >
                {feel}
              </button>
            ))}
